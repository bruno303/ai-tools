...
def _add_model_usage(usage: dict[str, int | float], payload: dict[str, Any]) -> None:
...
def parse_usage(events: Iterable[dict[str, Any]]) -> dict[str, int | float]:
    ...
    for event in events:
        ...
        if event_type in MODEL_EVENT_TYPES:
            _add_model_usage(usage, payload)
...
def _open_usage_parent(root_fd: int, components: tuple[str, ...]) -> tuple[int, list[int]]:
...
def _create_temporary_file(parent_fd: int, filename: str) -> tuple[int, str]:
    for _ in range(100):
...
def write_usage(root_fd: int, components: tuple[str, ...], usage: dict[str, Any]) -> None:
    ...
    parent_fd, directory_fds = _open_usage_parent(root_fd, components)
    try:
        filename = components[-1]
        fd, temporary_path = _create_temporary_file(parent_fd, filename)
        try:
...
def _consume_stdout(stream: TextIO, events: list[dict[str, Any]]) -> None:
    for line in stream:
...
def _consume_stderr(stream: TextIO) -> None:
    for line in stream:
        sys.stderr.write(line)
        sys.stderr.flush()


def _collect_process_output(process: subprocess.Popen[str]) -> list[dict[str, Any]]:
    events: list[dict[str, Any]] = []
    assert process.stdout is not None
    assert process.stderr is not None
    stdout_thread = threading.Thread(target=_consume_stdout, args=(process.stdout, events))
    stderr_thread = threading.Thread(target=_consume_stderr, args=(process.stderr,))
    stdout_thread.start()
    stderr_thread.start()
    process.wait()
    stdout_thread.join()
...
def main(argv: list[str] | None = None) -> int:
    ...
    events = _collect_process_output(process)
